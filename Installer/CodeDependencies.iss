...
var
  ...
  Dependency_Memo_Additional: String;
  Dependency_Memo_Additional_Task: String;                        
  Dependency_List: array of TDependency_Entry;
  Dependency_NeedRestart, Dependency_ForceX86: Boolean;
  Dependency_DownloadPage: TDownloadWizardPage;

procedure Dependency_Set_Additional_Memo(Text: String);
begin
  Dependency_Memo_Additional := Text;
end;

procedure Dependency_Set_Additional_Task_Memo(Text: String);
begin
  Dependency_Memo_Additional_Task := Text;
end;
...
begin
  ...
  if Dependency_Memo_Additional <> '' then
  begin
    Result := Result + FmtMessage(Dependency_Memo_Additional, [Space, NewLine]) + Newline + NewLine
  end;
  ...
  if Dependency_Memo <> '' then begin
    ...
    if Dependency_Memo_Additional_Task <> '' then begin
      Result := Result + Newline + FmtMessage(Dependency_Memo_Additional_Task, [Space]);
    end;
  end;
end;
